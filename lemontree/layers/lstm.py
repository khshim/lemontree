...
class LSTMRecurrentLayer(BaseRecurrentLayer):
    ...
    def __init__(self, input_shape, output_shape,
        ...
        W = np.zeros((input_shape[0], output_shape[0] * 4)).astype(theano.config.floatX)  # input[t] to ~~
        self.W = theano.shared(W, self.name + '_weight_W')
        self.W.tags = ['weight', self.name]
        # order: forget, input, output, cell_candidate
        U = np.zeros((output_shape[0], output_shape[0] * 4)).astype(theano.config.floatX)  # output[t-1] to ~~
        ...
        if self.peephole:
            ...
            V = np.zeros((output_shape[0], output_shape[0] * 3)).astype(theano.config.floatX)  # cell[t-1] to ~~
