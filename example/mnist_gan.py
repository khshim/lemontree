...
from lemontree.objectives import BinaryCrossentropy, CategoricalCrossentropy
...
train_gen = SimpleGenerator([train_data], 256, 'train')

#================Build graph================#

x = T.fmatrix('X')  # (batch_size, 784)
z = T.fmatrix('Z')  # (batch_size, 100)

generator = SimpleGraph(experiment_name + '_gen', batch_size)
generator.add_layer(DenseLayer((100,),(256,), use_bias=False), is_start=True) # 0
generator.add_layer(BatchNormalization1DLayer((256,)))                         # 1
generator.add_layer(ReLU())                                                     # 2
generator.add_layer(DenseLayer((256,),(784,)))                                  # 3
generator.add_layer(Sigmoid())                                                 # 4

fake, fake_layers = generator.get_output({0:[z]}, -1, 0)

discriminator = SimpleGraph(experiment_name + '_disc', batch_size)
discriminator.add_layer(DenseLayer((784,), (256,), use_bias=False), is_start=True) #0
discriminator.add_layer(BatchNormalization1DLayer((256,)))
discriminator.add_layer(ReLU(0.1))
discriminator.add_layer(DenseLayer((256,), (1,)))
...
disc_loss_real = BinaryCrossentropy(True).get_output(real_disc, T.ones_like(real_disc))
disc_loss_fake = BinaryCrossentropy(True).get_output(fake_disc, T.zeros_like(fake_disc))
disc_loss = disc_loss_fake + disc_loss_real
gen_loss = BinaryCrossentropy(True).get_output(fake_disc, T.ones_like(fake_disc))
...
disc_func = theano.function(inputs=[x,z],
                            outputs=[disc_loss, disc_loss_real, disc_loss_fake],
...
def pretrain_disc():
    ...
    for index in range(train_gen.max_index):
        ...
        train_outputs = disc_func(data, latent)
...
def train_trainset():
    ...
    for index in range(train_gen.max_index):
        ...
        train1_outputs = disc_func(data, latent1)
        train2_outputs = gen_func(latent2)
...
def generate_fake(epoch):
    ...
    latent = np.random.normal(0, 1, size=(batch_size, 100))
    fake = fake_image(latent)  # (256, 784)
    for a in range(10):
        for b in range(10):
            index = a * 10 + b
